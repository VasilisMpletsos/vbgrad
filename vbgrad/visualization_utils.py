...
def backtrace_connections(root_node: Parameter):
    visited_nodes, edges = set(), set()

    def find_connected_nodes(selected_node: Parameter):
...
def draw_graph(root_node: Parameter):
    graph = Digraph(format="svg", graph_attr={"rankdir": "LR"})
    nodes, edges = backtrace_connections(root_node)

    for node in nodes:
        uuid = str(id(node))
        graph.node(name=uuid, label=f"Parameter {node.value}:.4f", shape="box")
